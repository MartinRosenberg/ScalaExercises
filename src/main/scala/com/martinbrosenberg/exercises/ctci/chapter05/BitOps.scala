...
object BitOps {
  ...
  def updateBit(n: Int, i: Int, value: Boolean): Int =
    if (value) setBit(n, i)
    else clearBit(n, i)

  /** [MSB, end] */
  def clearHighBitsTo(n: Int, end: Int): Int =
    n & ((1 << end) - 1)

  /** [MSB, end) */
  def clearHighBitsUntil(n: Int, end: Int): Int =
    n & ((1 << (end + 1)) - 1)

  /** [start, 0] */
  def clearLowBitsFrom(n: Int, start: Int): Int =
    n & (-1 << (start + 1))

  /** (start, 0] */
  def clearLowBitsAfter(n: Int, start: Int): Int =
    n & (-1 << start)

...
/** STRING ROTATION
  ...
  * of `s1` using only one call to `isSubstring`. Assume you have a method
  * `isSubstring` which checks if one word is a substring of another.
  */
object E09_StringRotation {

  /** Constraints: None.
    *
    * Scala already contains `contains`, which accomplishes the same as
    * `isSubstring`. We just need to check that the strings are of the same

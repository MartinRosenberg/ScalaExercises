...
object E01_IsUnique {
  ...
  /** Constraints: You must detect duplicate characters manually instead of
    *   relying on built-ins.
    *
    * This makes a map with each key being a unique character, and each value a
    * collection of all instances of that key. Then `forall` takes the length of
  ...
  /** Constraints: You may only use an array, or a data structure you would use
    *   in the same way.
    *
    * Uses `java.util.BitSet` as a bitmap. Each bit defaults to false. The first
    * time we find a character, we set its bit to true. The second time, we
    * early return `false`. Otherwise, we return `true` at the end. Using
    * `scala.collection.mutable.BitSet` would be half as fast for no gain.
    *
    * This version is problematic in a few ways: compared to earlier solutions,
  ...
  /** Constraints: You may only use an array.
    *
    * Similar to the `BitSet` version, but clumsier. Uses a boolean array, big
    * enough for extended ASCII by default, and naïvely resized for new Unicode
    * characters.
    *
  ...
  /** Constraints: You may not use additional data structures.
    *
    * Interprets the constraint strictly, disallowing out-of-place sort.
    * Compares every character against every other  character, returning `false`
    * early upon finding a duplicate.
    *

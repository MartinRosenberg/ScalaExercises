...
import com.martinbrosenberg.exercises.codewars.EncryptThis_Spec.tests

class EncryptThis_Spec extends BaseSpec {

	for ((input, expected) <- tests) {
		s"encryptThis($input) should return $expected" in {
...
object EncryptThis_Spec {

	private final val tests = Map(
		"Hello" -> "72olle",
		"good" -> "103doo",
